...
export default function PurchaseOrderDetailScreen() {
  ...
  const handleAction = (action: "complete" | "cancel" | "reverse") => {
    ...
    Alert.alert("确认操作", `确定${labels[action]}该采购单？`, [
      { text: "取消", style: "cancel" },
      {
        text: "确定",
        onPress: async () => {
          try {
            if (action === "complete") await transactionApi.completePurchaseOrder(Number(id));
            if (action === "cancel") await transactionApi.cancelPurchaseOrder(Number(id));
            if (action === "reverse") await transactionApi.reversePurchaseOrder(Number(id));
            loadOrder();
          } catch (err) {
            console.error(err);
          }
  ...
  if (!data) {
    return (
      <View style={styles.center}>
        <Text style={styles.errorText}>采购单不存在</Text>
      </View>
    );
  }
