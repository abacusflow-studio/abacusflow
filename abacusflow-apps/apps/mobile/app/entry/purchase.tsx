...
export default function PurchaseEntryScreen() {
  ...
  const restoreDraft = async (id: string) => {
    const drafts = await listDrafts("purchase");
    const draft = drafts.find((d) => d.id === id);
    if (!draft) return;
    const p = draft.payload;
    setSelectedPartnerId(p.supplierId as number | undefined);
    setOrderDate((p.orderDate as string) || orderDate);
    setItems((p.items as OrderItem[]) || []);
  ...
  const autoSaveDraft = useCallback(
    async (
      ...
      if (draftId) {
        ...
        await updateDraft("purchase", draftId, { payload, summary });
      } else {
        const draft = await saveDraft("purchase", payload, summary);
        setDraftId(draft.id);
      }
    },
    [draftId, orderDate],
  ...
  const addItem = (product: SelectableProduct) => {
    const newItems = [
      ...items,
      {
    ...
    setItems(newItems);
    autoSaveDraft(newItems, selectedPartnerId, note);
  };
  ...
  const handleScan = useCallback(
    (barcode: string) => {
      ...
      if (product) {
        if (items.some((i) => i.productId === product.id)) {
          Alert.alert("提示", `「${product.name}」已在明细中`);
          return;
        }
        addItem(product);
      } else {
    ...
    [products, items, selectedPartnerId, note],
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (draftId) await deleteDraft("purchase", draftId);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          {/* Step 1: Supplier */}
          <Text style={styles.stepLabel}>1. 选择供应商</Text>
          <ScrollView
            horizontal
          ...
          >
            ...
            <Text style={styles.scanBtnText}>扫码添加产品</Text>
          </TouchableOpacity>
          ...
          {showMore && (
            <>
              ...
              <TextInput
                ...
                onChangeText={(v) => {
                  setNote(v);
                  autoSaveDraft(items, selectedPartnerId, v);
                }}
                placeholder="可选备注"
              />
            </>
        ...
        <View style={styles.bottomBar}>
          ...
          >
            {submitting ? (
              ...
              <Text style={styles.submitText}>提交入库</Text>
            )}
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
