    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.card}>
          <InfoRow label="订单日期" value={dateToFormattedString(data.orderDate)} />
          <InfoRow label={partnerLabel} value={partnerName} />
          {extraFields?.map((field) => (
            <InfoRow key={field.label} label={field.label} value={field.value} />
          ))}
          {data.totalAmount != null && (
            <InfoRow label="总金额" value={`¥${data.totalAmount.toLocaleString("zh-CN")}`} valueStyle={styles.amount} />
          )}
        </View>
        ...
        <View style={styles.card}>
          {data.items.map((item, idx) => (
            <OrderItemRow key={item.id ?? idx} item={item} isLast={idx === data.items.length - 1} />
          ))}
        </View>
        ...
        <View style={styles.actions}>
          {data.status === "pending" && onComplete && onCancel && (
            <>
              <TouchableOpacity style={[styles.actionBtn, { backgroundColor: COLORS.success }]} onPress={() => handleAction("complete")}>
              ...
              <TouchableOpacity style={[styles.actionBtn, styles.cancelBtn]} onPress={() => handleAction("cancel")}>
                <Text style={[styles.actionBtnText, { color: COLORS.danger }]}>取消</Text>
              </TouchableOpacity>
            </>
          )}
          {data.status === "completed" && onReverse && (
            <TouchableOpacity style={[styles.actionBtn, styles.reverseBtn]} onPress={() => handleAction("reverse")}>
...
const styles = StyleSheet.create({
  ...
  itemPrice: { fontSize: 14, color: COLORS.primary, fontWeight: "600", marginTop: 2 },
  actions: { flexDirection: "row", gap: 12, marginTop: 8 },
  actionBtn: { flex: 1, paddingVertical: 14, borderRadius: 8, alignItems: "center" },
  actionBtnText: { color: "#fff", fontSize: 15, fontWeight: "600" },
  cancelBtn: { backgroundColor: "#fff1f0", borderWidth: 1, borderColor: "#ffccc7" },
  reverseBtn: { backgroundColor: "#f5f5f5", borderWidth: 1, borderColor: "#d9d9d9" },
});
