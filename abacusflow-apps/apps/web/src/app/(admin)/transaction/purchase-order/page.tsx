...
export default function PurchaseOrdersPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await transactionApi.listPurchaseOrdersPage({
        pageIndex,
        pageSize: 10,
  ...
  const handleAction = async (id: number, action: "complete" | "cancel" | "reverse") => {
    ...
    if (!confirm(`确定${labels[action]}该采购单？`)) return;
    try {
      if (action === "complete") await transactionApi.completePurchaseOrder(id);
      if (action === "cancel") await transactionApi.cancelPurchaseOrder(id);
      if (action === "reverse") await transactionApi.reversePurchaseOrder(id);
      fetchData();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div>
      <PageHeader
        title="采购单管理"
        extra={<Button type="primary" label="新增采购单" onClick={() => {}} />}
      />
      <div className="card">
