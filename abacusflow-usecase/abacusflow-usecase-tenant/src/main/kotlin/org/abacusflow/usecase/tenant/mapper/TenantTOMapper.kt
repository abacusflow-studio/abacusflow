...
fun TenantMembership.toTO(userName: String = "Unknown User") =
    TenantMembershipTO(
        id = id,
        tenantId = tenantId,
        userId = userId,
        userName = userName,
        status = status.name,
        roleNames = tenantRoles.map { it.name },
    )

fun TenantMembership.toSummaryTO(tenant: Tenant) =
    TenantSummaryTO(
        tenantId = tenant.id,
        name = tenant.name,
        displayName = tenant.displayName,
        roleNames = tenantRoles.map { it.name },
        permissionNames = tenantRoles.flatMap { it.permissions }.map { it.name }.distinct(),
    )
